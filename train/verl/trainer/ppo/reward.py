...
def load_reward_manager(
    ...
    # Handle sentence transformer configuration
    sentence_transformer_config = config.reward_model.get("sentence_transformer")
    sentence_transformer_url = sentence_transformer_config.get("url") if sentence_transformer_config else None
    sentence_transformer_timeout = sentence_transformer_config.get("timeout") if sentence_transformer_config else None
    sentence_transformer_threshold = sentence_transformer_config.get("similarity_threshold") if sentence_transformer_config else None
    
    if sentence_transformer_url:
        sentence_transformer_manager = multiprocessing.Manager()
        # Create a semaphore to control concurrent access to sentence transformer API
        _sentence_transformer_semaphore = sentence_transformer_manager.Semaphore(
            sentence_transformer_config.get("max_concurrent", 64)
        )
        # Wrap the existing compute_score with sentence transformer parameters
        current_compute_score = final_compute_score
        final_compute_score = partial(
            current_compute_score,
            sentence_transformer_url=sentence_transformer_url,
            sentence_transformer_timeout=sentence_transformer_timeout,
            sentence_transformer_threshold=sentence_transformer_threshold,
            sentence_transformer_semaphore=_sentence_transformer_semaphore,
        )
