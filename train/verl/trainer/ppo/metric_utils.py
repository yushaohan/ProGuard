...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> dict[str, Any]:
    ...
    # Add data_source-specific reward metrics
    if "data_source" in batch.non_tensor_batch:
        data_sources = batch.non_tensor_batch["data_source"]
        # Group rewards by data_source
        data_source_rewards = defaultdict(list)
        for i, data_source in enumerate(data_sources):
            if i < len(non_aborted_sequence_reward):
                data_source_rewards[data_source].append(non_aborted_sequence_reward[i].item())
        
        # Calculate mean reward for each data_source
        for data_source, rewards in data_source_rewards.items():
            if rewards:  # Only add metric if there are rewards for this data_source
                metrics[f"critic/rewards/{data_source}/mean"] = np.mean(rewards)
